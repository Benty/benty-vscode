...
import { log, readToJson, styles, toRelativePath, writeToFile, } from '../utilities.js';
const parseVariant = (currentVariant, { name, type, ...baseTheme }, { options, variants, colors }) => {
    ...
    const theme = Object.keys(colors).reduce((result, currentColor) => {
        ...
        const undefinedColor = () => {
            log.warn(`
				Unable to find ${styles.string(currentColor)} in colors. Check that color exists, e.g.
        ...
        const colorValue = typeof color === 'string' ? color : color[currentVariant];
        if (typeof colorValue === 'undefined') {
            return undefinedColor();
        }
        return result.replace(new RegExp(escapeStringRegexp(`"${options.prefix}${currentColor}"`), 'g'), `"${colorValue}"`);
    ...
    if (typeof workingColors !== 'undefined') {
        for (const key of workingColors) {
            if (typeof parsedTheme.colors !== 'undefined' &&
    ...
    writeToFile(path.join(toRelativePath(options.output), `${slug}-color-theme.json`), JSON.stringify(parsedTheme, null, '\t'));
    if (options.includeNonItalicVariants) {
        const removeItalics = JSON.stringify(parsedTheme).replace(/\s?italic\s?/g, '');
        const parsedNonItalicTheme = {
            ...JSON.parse(removeItalics),
            name: `${variant.name} (no italics)`,
        };
        writeToFile(path.join(toRelativePath(options.output), `${slug}-no-italics-color-theme.json`), JSON.stringify(parsedNonItalicTheme, null, '\t'));
    }
    return variant.name;
};
export const build = (config) => {
    ...
    if (!source.includes('-color-theme')) {
        console.log(`🌸 Maybe rename ${styles.string(source)} to ${styles.string(source.replace('.json', '-color-theme.json'))}\n   for improved code completions, color decorators, and color pickers when editing.\n`);
    }
    const template = readToJson(source);
